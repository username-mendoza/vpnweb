...
# Designed to run behind HAProxy (SSL termination on HAProxy host).
# Backend listens on plain HTTP port 8080.
...
if systemctl is-active --quiet "${SERVICE_NAME}"; then
    ...
    green "    Service: http://0.0.0.0:${PORT} (HTTP, plain)"
    green "    Put HAProxy in front with SSL termination on the HAProxy host."
    green "    HAProxy backend: http://<this-host>:${PORT}"
else
